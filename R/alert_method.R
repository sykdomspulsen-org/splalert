...
create_train_prediction_data <- function(d){
  ...
  prediction_isoyearweek <- isoyearweek[(length(isoyearweek)-52):length(isoyearweek)]
  train_isoyearweek <- isoyearweek[(length(isoyearweek)-52*5):(length(isoyearweek)-(52))-1]


  d_train <- d[isoyearweek %in% train_isoyearweek]
  d_prediction <- d[isoyearweek %in% prediction_isoyearweek]

  return(list(d_train=d_train,d_prediction=d_prediction))
}


#' Quasipoisson Algorithm
#'
...
#' @import data.table
#' @export
quasipoisson_core_skuhr_create_alerts <- function(data,
                                                  reweights = 1,
                                                  remove.highcounts = 0,
                                                  is_day = T) {
