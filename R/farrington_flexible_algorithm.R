...
#'
#'
#' Description: Noufaily et al., 2013. The algorithm fits a quasi-possion regression model. Seasonality is accounted by including a yearly 10-level factor. Linear trend is always included
#'
#' Input (arguments):
...
#' @import data.table
#' @export
farrington_flexible_algorithm <- function(data,
                                 reweights = 1,
                                 remove.highcounts = 0,
                                 is_day = T) {
  ...
  if (poisreg$failed) {
      ...
      try(
        {
          ...
          anscombe.res[anscombe.res < 2.58] <- 1 # Alt. 2.58?
    ...
    if (pred$failed) {
      ...
      d_prediction[, baseline_n_predinterval_l4 := farrington_threshold(pred$vals, phi = dispersion_parameter, z = -4, skewness.transform = "2/3")]
      d_prediction[, baseline_n_predinterval_l2 := farrington_threshold(pred$vals, phi = dispersion_parameter, z = -2, skewness.transform = "2/3")]
      d_prediction[, baseline_n_predinterval_u2 := farrington_threshold(pred$vals, phi = dispersion_parameter, z = 2, skewness.transform = "2/3")]
      d_prediction[, baseline_n_predinterval_u4 := farrington_threshold(pred$vals, phi = dispersion_parameter, z = 4, skewness.transform = "2/3")]
      d_prediction[, zscore := farrington_zscore(pred$vals, phi = dispersion_parameter, z = 6, skewness.transform = "2/3", y = n_tot)]
      d_prediction[, status := "normal"]
...
#' Calculate Farrington Flexible SE in gamma space
...
farrington_flexible_se_gamma_space <- function(pred, phi, alpha = NULL, z = NULL, skewness.transform = "none") {
  mu0 <- pred$fit
...
# Calculate Farrington Flexible threshold
# @param pred Point estimate
# @param phi Dispersion
# @param alpha Alpha (e.g 0.05)
# @param z Similar to \code{alpha} (e.g. 1.96)
# @param skewness.transform "none"/"1/2","2/3"
farrington_flexible_threshold <- function(pred, phi, alpha = NULL, z = NULL, skewness.transform = "none") {
  mu0 <- pred$fit
  tau <- phi + (pred$se.fit^2) / mu0
  switch(skewness.transform,
         none = {
...
# @param skewness.transform "none"/"1/2","2/3"
# @param y Observation
farrington_flexible_zscore <- function(pred, phi, alpha = NULL, z = NULL, skewness.transform = "none", y) {
  mu0 <- pred$fit
  tau <- phi + (pred$se.fit^2) / mu0
  switch(skewness.transform,
         none = {
